(scope => {
	...
	WebGLRenderer.prototype.flushVertexBuffer = function (clip) {
		...
		if (this.vertexClip !== clip || !clip || this.drawAreaChange) {
			gl.enable(gl.SCISSOR_TEST);
			if (clip) {
				const dah = (this.drawAreaB - this.drawAreaT + 1) * qhf;
				const dat = this.drawAreaT * qhf;
				const daw = (this.drawAreaR - this.drawAreaL + 1) * qwf;
				gl.scissor(this.drawAreaL * qwf, dat, daw, dah);
			}
			else
