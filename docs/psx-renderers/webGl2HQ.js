...
function WebGLRenderer(cv) {
  ...
  if (gl) {
    ...
    this.programRenderer = createProgramRenderer(gl, this.displayBuffer);
...
WebGLRenderer.prototype.drawTriangle = function (data, c1, xy1, c2, xy2, c3, xy3, tx, ty, uv1, uv2, uv3, cl) {
  ...
  var x3 = $gpu.daX + ((data[xy3] << 21) >> 21);
  var y3 = $gpu.daY + ((data[xy3] << 5) >> 21);
  ...
  // if (!textured) {
  var buffer = vertexBuffer;//this.getVertexBuffer(3, data[0]);
  buffer.addVertex(x1, y1, 0, 0, data[c1]);
  buffer.addVertex(x2, y2, 0, 0, data[c2]);
  buffer.addVertex(x3, y3, 0, 0, data[c3]);
  // }
}
...
WebGLRenderer.prototype.drawRectangle = function (data, tx, ty, cl) {
  ...
  buffer.addVertex(x + 0, y + 0, 0, 0, c);
  buffer.addVertex(x + w, y + 0, 0, 0, c);
  buffer.addVertex(x + 0, y + h, 0, 0, c);

  buffer.addVertex(x + w, y + 0, 0, 0, c);
  buffer.addVertex(x + 0, y + h, 0, 0, c);
  buffer.addVertex(x + w, y + h, 0, 0, c);
  // }
}
...
function flushVertexBuffer(renderer) {
  ...
  if (vertexBuffer.index) {
    ...
    gl.drawArrays(gl.TRIANGLES, 0, vertexBuffer.index / 12);
